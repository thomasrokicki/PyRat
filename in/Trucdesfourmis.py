...
def dijkstra(mazeMap,beginningNode):
    ...
    while heap !=[] :                                                          
        ...
        for neighbor in mazeMap[currentNode]:                             # we must find a path with the lowest weight among all the neighbors    
...
def metaGraph(mazeMap,piecesOfCheese,beginningNode):                                #Creates the metagrapĥ of the pieces of cheeses in o(n³). mazeMap is the initial dict which represents the maze 
    metaGraph = {}
    listOfRoutes = {}                                                               #is a dict containing all the route tables
    (route,distances) = dijkstra(mazeMap, beginningNode)                            
    metaGraph[beginningNode] = {}                                                   # dict of dict to contain the path to all the cheeses and define a metagraph
    for cheese in piecesOfCheese :                                                  #for each piece of cheese, we add the path to the other pieces and the weight of this path to get a compleete graph
        metaGraph[beginningNode][cheese] = distances[cheese]                        
        listOfRoutes[beginningNode] = route                                         
    for currentNode in piecesOfCheese :
        (route,distances) = dijkstra(mazeMap,currentNode)                           #We use dijsktra to get the shortest path between each piece of cheese.                        
    ...
    return (metaGraph,listOfRoutes)                                                # we return the metaGraph and a list of routes necessary to get the shortest path between the pieces of cheese.
            
                
    
def initialisationpheromone(metagraph):                         
    pheromone = {}
    for pos1 in metagraph:
        pheromone[pos1] = {}
        for pos2 in metagraph:
            pheromone[pos1][pos2] = 0
    return pheromone
 
def letpheromone(pheromone, metagraph, beginningNode ,finalNode):  # The ant let pheromone (1/(length of travel)) after travelling between i and j
    # with pheromone : a dict of coordinates of nodes where ants have let pheromone 
    distance = metagraph[beginningNode][finalNode]
    pheromone[beginningNode][finalNode] = 1/distance
    pheromone[finalNode][beginningNode] = 1/distance

def evaporationOfpheromone(pheromone,k,metagraph) :             # k a factor representing the evaporation
    for beginningNode in metagraph:
        for finalNode in metagraph:
            if finalNode != beginningNode:
                pheromone[beginningNode][finalNode]=k*pheromone[beginningNode][finalNode]
    return
                
def choiceOfTheNextNode(pheromone,metagraph,beginningNode):                         # Next Node to visit for each ants                    
    pheromonetoadd,totalpheromone = 0,0
    for nodes in metagraph[beginningNode] :
        pheromonetoadd = 1/metagraph[beginningNode][nodes] + pheromonetoadd
        totalpheromone = pheromone[beginningNode][nodes] + totalpheromone
    length = {}
    for nodes in metagraph[beginningNode]:
        length[nodes] = (1/5)* 1 / (pheromonetoadd*metagraph[nodes][beginningNode]) + 4/5 * pheromone[nodes][beginningNode]/totalpheromone      # theoric formula allowing to find the optimal length  
    p = random()
    l = 0
    
    for node in metagraph[beginningNode] :                                                                                                       # and therefore the next node
        if p>l:
            l+=lenght[node]
        else :
            return node

def choiceFinalAnt(pheromone,metagraph,beginningNode):                              # To find which Node to go finally                            
    lenghtmax = 0               
    finalNode = None
    for nodes in metagraph[beginningNode] :
        lenghtmax = max(pheromone[beginningNode][nodes],lenghtmax)
        if lenghtmax == pheromone[beginningNode][nodes] :
            finalNode = nodes
    return finalNode
        
def antsProgram(metagraph,nbOfAnts,piecesOfCheese,beginningNode):
    pheromone = initialisationpheromone(metagraph)
    nextNode = [beginningNode]
    for nodes in metagraph[beginningNode]:
        for i in range(nbOfAnts):  
            otherNodes = [x for x in metagraph if x != nodes] 
            while otherNodes != []:                                                               
                letpheromone(pheromone, metagraph, nodes ,nextNode) 
                nextNode = choiceOfTheNextNode(pheromone,metagraph,nodes)                                                                 
                othernodes.pop(othernodes.index(nextNode))
            letpheromone(pheromone, metagraph, nextNode ,beginningNode)
        evaporationOfpheromone(pheromone,0.9,metagraph)
    path = [beginningNode] 
    for nodes in metagraph[beginningNode]:
        next = choiceFinalAnt(pheromone,metagraph,nodes)                            
        resultat.append(next)
    return resultat
...
def preprocessing (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed) :
    ...
    bestPath = antsProgram(graph,5,piecesOfCheese,playerLocation)
    
    global finalPath

    for k in range (len(bestPath)-1) :                                          # We define our finalPath (the "shortest") with our lists
