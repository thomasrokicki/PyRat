...
def closestPieceOfCheese (piecesOfCheese, currentLocation, metaGraph) :        #Return the closest piece of cheese from currentLocation, distances is the dict of distances returned by dijkstra
    cand = (0,0)
    print(type(metaGraph))
    bestLength = inf
    for cheese in piecesOfCheese :
        if cheese != currentLocation :
            if metaGraph[currentLocation][cheese] <= bestLength :
                cand = cheese
                bestLength = metaGraph[currentLocation][cheese]
    return cand


def greedyAlgorithm(beginningNode, piecesOfCheese, metaGraph) :                             #A greedy algorithm going to the closest piece of cheese each time
    currentLocation = beginningNode
    
    pathLength = 0
    path = [currentLocation]
    while piecesOfCheese != [] : 
        print(type(metaGraph))                                                           #goes to the closest piece of cheese each time, creating the path
        cheese = closestPieceOfCheese(piecesOfCheese, currentLocation, metaGraph)
        path.append(cheese)
        pathLength += metaGraph[currentLocation][cheese]
...
def backTrackingTravellingSalesman (metaGraph, beginningNode, piecesOfCheese) :                                 #Optimization of Travelling Salesman using backtracking, the first branch being initialized by the greedy algorithm above
    (bestPath, bestLength) = greedyAlgorithm(beginningNode, piecesOfCheese, metaGraph)
    def exhaustive (remainingNodes, currentNode, currentPath, currentLength, graph) :               # recursive program to deal with all the neighbors with an exhaustive process
        ...
        if remainingNodes == [] :
            ...
            for neighbor in graph[currentNode] :                                                                     # all the neighbors are treated if they are actually the relevant neighbor
                
                weight = graph[currentNode][neighbor]
                if neighbor in remainingNodes :                                                                       
                    otherNodes = [x for x in remainingNodes if x != neighbor]
                    exhaustive(otherNodes, neighbor, currentPath + [neighbor], currentLength + weight, graph)        # exhaustive process to finish the path dealing with the remaining neighbors
    otherNodes = [x for x in metaGraph if x != beginningNode]                                                        # We define our relevant neighbors
...
def preprocessing (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed) :
    ...
    (bestPath, bestLength) = backTrackingTravellingSalesman(graph,playerLocation,piecesOfCheese)
    print(bestPath)
    global finalPath
