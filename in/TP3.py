...
def preprocessing (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed) :
    ...
    #listOfRoutes=[dijkstra(mazeMap,piecesOfCheese[k]) for k in range(len(piecesOfCheese))]
    global listOfPath
    listOfPath = []
    location=playerLocation
    
    while piecesOfCheese != [] :
        closestPieceOfCheese = 0
        for cheese in range(len(piecesOfCheese)) :
            if distances[piecesOfCheese[cheese]] < distances[piecesOfCheese[closestPieceOfCheese]] :
                closestPieceOfCheese = cheese
        path = shortestPath(route,location,piecesOfCheese[closestPieceOfCheese])
        listOfPath += path
        
        location = piecesOfCheese[closestPieceOfCheese]
        (route,distances) = dijkstra(mazeMap, location)
        piecesOfCheese.pop(closestPieceOfCheese)
