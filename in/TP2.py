...
import numpy  #For moveFromLocations

#Algorithm from TP1, returning the move between two adjacent cases of the maze

def moveFromLocations (sourceLocation, targetLocation) : 
    difference = tuple(numpy.subtract(targetLocation, sourceLocation))          #substract makes the substraction betweent the coordinate's tuples of the two cases 
    if difference == (1, 0) :
        return MOVE_DOWN
    elif difference == (-1, 0) :
        return MOVE_UP
...
#Elementary function relative to queues
def createFIFO():
    return []

def isEmptyFIFO(queue):
    return queue == []


def pushFIFO(element,queue):
...
def breadthFirstSearch(mazeMap,beginningNode):                 
    route={}                                                   #Hash table stocking the routing for our BFS
    for pos in mazeMap:                                        #We shape the hash table
        route[pos]=None                                        #With empty blocks
    dejaVu=[beginningNode]                                     #List of visited nodes
    queue=createFIFO()                                         #Creation of the queue for the BFS
    queue=pushFIFO(beginningNode,queue) 
    while not(isEmptyFIFO(queue)):                             #Start of the BFS
        currentNode=popFIFO(queue)                             #For each node in the queue
        for neighbor in mazeMap[currentNode]:                  #We visit its neighbors
            if neighbor not in dejaVu :                        #and, if not visited, we add them to the queue and dejaVu
                dejaVu.append(neighbor)
                queue=pushFIFO(neighbor,queue)
                route[neighbor]=currentNode
    print(dejaVu)
    print(route)
    return route                                              #We return the routing




#Program returning a list of the coordinates of the successives nodes composing the shortest path
...
def preprocessing (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed) :
    route=breadthFirstSearch(mazeMap,playerLocation)                          # We first create the routing from the initial position of the player
    global path                                                               # We create a global list
    path=shortestPath(route,playerLocation,piecesOfCheese[0])                 # Filled with the shortest path (still as coordinate's couples)
    print(path)
