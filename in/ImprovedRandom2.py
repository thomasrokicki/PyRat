...
def randomMoveUpgraded (mazeMap,fromLocation) :
    moves = []
    if getMove(mazeMap,fromLocation,leftOf(fromLocation))!=None:
        moves.append(MOVE_LEFT)
    if getMove(mazeMap,fromLocation,rightOf(fromLocation))!=None:
        moves.append(MOVE_RIGHT)
    if getMove(mazeMap,fromLocation,aboveOf(fromLocation))!=None:
        moves.append(MOVE_UP)
    if getMove(mazeMap,fromLocation,belowOf(fromLocation))!=None:
        moves.append(MOVE_DOWN)
    l=len(moves)
    print(moves)
    return moves[random.randint(0, l-1)]
...
def turn (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese, timeAllowed) :
    ...
    if moves :
        ...
        return randomMoveUpgraded(mazeMap,playerLocation)
