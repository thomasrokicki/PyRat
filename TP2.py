...
def breadthFirstSearch(mazeMap,beginningNode):
    ...
    for pos in mazeMap:
        route[pos]=None
    dejaVu=[beginningNode]
    queue=createFIFO()
    queue=pushFIFO(beginningNode,queue)
    while not(isEmptyFIFO(queue)):
        currentNode=popFIFO(queue)
...
def preprocessing (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed) :
    route=breadthFirstSearch(mazeMap,playerLocation)
    global path
...
def turn (mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese, timeAllowed) :
    move = 'X'
    if len(path)>1:  
        move = moveFromLocations(path[0],path[1])
        path.pop(0)
    print(move)
    return move
